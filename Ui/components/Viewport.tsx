...
const handleHelperMode = () => three.toggleHelpers()
...
const Viewport: React.FC<ViewportProps> = ({ mode, setMode, selectedObject }) => {
    ...
    useEffect(() => {
        ...
        if (!el.hasChildNodes()) el?.appendChild?.(editor.core.engine.getCanvas())
    ...
    const getDynamicTools = (): DynamicTool[] => {
        ...
        if (selectedObject.isObject3D) {
            tools.push({ id: 'focus', icon: Target, tooltip: 'Focus Object (F)', action: () => editor.api.three.focusSelected() });
        }
