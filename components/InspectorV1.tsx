...
import { Vector3Input, Checkbox, ColorInput, NumberInput, TextInput, AssetInput, RefInput, MaterialGeoRefInput, TextureInput } from './PropertyInputs';
...
const RenderPropertyInput: React.FC<RenderPropertyInputProps> = React.memo(({ path: key, val: v2, recursive }) => {
    ...
    const val = useObserver(object, key); // perfect mutation update rerenderer
...
const Inspector: React.FC = () => {
    ...
    const { selectedObject: object, mappedObject } = useEditorStates();
    // want to remove mapped object feature 
    ...
    return (
        <div className="h-full flex flex-col bg-editor-panel overflow-y-auto" key={object.uuid}>
            ...
            <div className="p-1 pb-10">
                ...
                {Object.keys(mappedObject?.map || {}).length > 0 && (
                    ...
                    >
                        {Object.entries(mappedObject.map).map(([key, val]) => (
                            <RenderPropertyInput key={key} path={key} val={val} recursive />
                        ))}
                    </PropertySection>
                )}
                ...
                {object.isObject3D&&<div className="px-3 mt-4">
                    <button
