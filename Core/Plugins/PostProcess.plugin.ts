import {type PostProcessCallback,  type PostProcessUpdate,  type PostProcessor} from "@/Core/three/Engine"
import {Plugin} from "@/Core/Plugins/Plugin"


import { THREE, TSL} from '@/Core/lib/THREE'
const { pass, uniform, renderOutput } = TSL;
...
export default class PostProcessingPlugin extends Plugin {
    ...
    async init() {
        ...
        // Get postProcessor from core
        this.postProcessor = this.app.engine.postProcessor;
        // Register built-in post-processing effects
        this.registerDefaultPasses();

        // What this plugin provides to others
        this.app.pluginData.postProcessing = {
    ...
    private registerPass(
        ...
        this.postProcessor.registerPostProcess(name, callback, active, order, data, update );
    }

    enablePass(name: string) {
        this.postProcessor.enablePostProcess(name);
    }

    disablePass(name: string) {
        this.postProcessor.disablePostProcess(name);
    }

    togglePass(name: string): boolean {
        return this.postProcessor.togglePostProcess(name);
    }

    updatePassData(name: string, data: any) {
        this.postProcessor.updatePostProcessData(name, data);
    }

    setPassOrder(order: string[]) {
        this.postProcessor.setPostProcessingOrder(order);
    }

    getRegistry() {
        return this.postProcessor.getPostProcessingRegistry();
    }

    isPassActive(name: string): boolean {
        return this.postProcessor.isPostProcessActive(name);
    }

    getPassData(name: string): any {
        return this.postProcessor.getPostProcessData(name);
    }
}